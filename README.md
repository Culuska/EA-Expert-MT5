...
Built and delivered in this order, each stage independently verifiable
before the next is trusted:

| Stage | What | Where |
|---|---|---|
| 1. BREG definition | Plain-language rules, no code — the contract everything else is judged against | [`docs/01_STRATEGY_DEFINITION.md`](docs/01_STRATEGY_DEFINITION.md) |
| 2. Visual BREG detector | Detects and draws Break/Retest/Engulfing on the chart. **No orders, nothing financial.** | [`MQL5/Experts/BREG/stages/02_BREG_Visual_Detector.mq5`](MQL5/Experts/BREG/stages/02_BREG_Visual_Detector.mq5) |
| 3. Backtestable BREG engine | Adds SL/TP calculation + **virtual (simulated, no real orders)** trade tracking and win/loss stats per timeframe | [`MQL5/Experts/BREG/stages/03_BREG_Backtest_Engine.mq5`](MQL5/Experts/BREG/stages/03_BREG_Backtest_Engine.mq5) |
| 4–7. Execution, risk, multi-TF, advanced filters | Real order placement, position sizing, daily/loss limits, independent per-timeframe state machines, optional HTF/liquidity/displacement/scoring filters | [`MQL5/Experts/BREG/BREG_EA.mq5`](MQL5/Experts/BREG/BREG_EA.mq5) — see the [module table](#architecture) below for which lines belong to which of these four stages |
| 8. Optimization | How to run the Strategy Tester optimizer per timeframe without overfitting | [`docs/08_OPTIMIZATION_GUIDE.md`](docs/08_OPTIMIZATION_GUIDE.md) |
| 9. Demo | Checklist before/during demo deployment | [`docs/09_DEMO_DEPLOYMENT.md`](docs/09_DEMO_DEPLOYMENT.md) |
| 10. Live | Go/no-go gate before real capital | [`docs/10_LIVE_DEPLOYMENT.md`](docs/10_LIVE_DEPLOYMENT.md) |

Stages 4–7 are delivered as one file rather than four separate ones because
they're genuinely interdependent — risk sizing needs execution to exist,
multi-timeframe needs per-timeframe risk caps, and the advanced filters
gate the scoring that execution reads — faking a clean split there would
just mean three files that don't run without the fourth. Stages 1–3,
8, 9 and 10 are each independently meaningful on their own, so they're
real, separate artifacts you can look at (or run) in isolation.
...
Every row below also exists (in a lighter form) in Stage 2/3 — the
**Stage** column says which build-order stage first introduced it in
`BREG_EA.mq5`, so you can trace any function back to why it exists.

| Module | Stage | Responsibility |
|---|---|---|
| `InitializeEA` | 2 | Builds the active timeframe list, creates ATR/MA indicator handles, configures `CTrade`, parses the manual news list. |
| `DetectSwingStructure` / `FindSwingHigh` / `FindSwingLow` / `IsValidSwingHigh` / `IsValidSwingLow` | 2 | Fractal swing detection with `Swing_Left_Bars`/`Swing_Right_Bars` confirmation and a `Minimum_Swing_Distance` prominence filter so noise isn't mistaken for structure. Adapts per timeframe because the same left/right/distance inputs are applied independently to each timeframe's own bar data. The validity check is factored out so it can be reused by both break detection and TP structure-target scanning. |
| `DetectBreak` / `ValidateBreak` / `CreateSetupFromBreak` | 2 | Confirms a **closed-bar** break beyond the swing level by at least `Minimum_Break_Distance` points (wicks alone never qualify), optionally requires displacement, records the break level/time, and opens a new `BregSetup` record. |
| `DetectRetest` / `ValidateRetest` | 2 | Waits (up to `Retest_Max_Bars`) for price to return into a tolerance zone around the broken level (`Retest_Tolerance_Points`, bounded by `Retest_Min_Depth`/`Retest_Max_Depth`). A strong closed-bar push back through the level by more than the tolerance + `Retest_Invalidation_Buffer_Points` cancels the setup early. |
| `DetectEngulfing` / `EvaluateEngulfing` / `CheckIntrabarEngulfing` | 2 (base) / 4 (intrabar mode) | Confirms a full-body engulfing candle within `Engulfing_Max_Bars_After_Retest` bars of the retest. `Use_Strict_Engulfing` gates both `Engulfing_Min_Body_Ratio` (vs. the previous candle) and `Engulfing_Min_Avg_Body_Ratio` (vs. recent average candle size, filtering out "weak" engulfs). The intrabar variant re-runs the same check against the still-forming bar for `Entry_Mode = INTRABAR_AGGRESSIVE`. |
| `CalculateStopLoss` / `CalculateTakeProfit` / `FindNextTPTarget` | 3 | Four SL placement modes (`STRUCTURE`, `ENGULFING_WICK`, `RETEST_SWING`, `ATR`) plus buffer. TP defaults to the nearest qualifying opposing structure/liquidity level (`TP_NEXT_STRUCTURE`), falling back to `RiskReward` × realized SL distance when no level qualifies or `TP_Method = TP_FIXED_RR`. Identical logic to Stage 3's virtual engine, now feeding real orders. |
| `ExecuteTrade` | 4 | Final permission/spread/session/news gate, stop-level clamping, volume normalization, and the actual `CTrade` market order. |
| `CheckSymbolTradeMode` / `ClampLotToFreeMargin` / `SendOrderWithRecovery` | 4 | Broker trade-mode awareness, margin-checked lot sizing, and bounded retry on requote/invalid-stops order failures. |
| `CalculateLotSize` / `CheckRiskLimits` / `CanOpenNewTrade` | 5 | Position sizing from `SYMBOL_TRADE_TICK_VALUE`/`SYMBOL_TRADE_TICK_SIZE`/volume step-min-max — no hard-coded per-symbol assumptions. Enforces `Max_Trades_Per_Day`, `Max_Open_Trades`, `Max_Consecutive_Losses`. |
| `ManageTrade` | 5 | Sanity-checks open EA positions each tick. |
| Per-timeframe `BregSetup`/magic-number partitioning, `One_Trade_Total` / `One_Trade_Per_Timeframe` | 6 | Each timeframe trades under its own `MagicNumber + tfIndex`, which is what makes independent per-timeframe state *and* per-timeframe trade accounting possible at the same time. |
| `CalculateSetupScore` | 7 | 0–110 point score: Break +30, Retest quality +5..+25, Engulfing quality +20..+25 (12 only reachable with `Use_Strict_Engulfing = false`), HTF alignment +10, Liquidity sweep +10, Displacement +10. Setups below `Minimum_Setup_Score` are invalidated instead of traded. |
| `CheckHTFContext` | 7 | Optional HTF EMA/price bias check (see above) — off by default per Stage 1. |
| `CheckLiquidity` | 7 | Optional PDH/PDL, equal-highs/lows and wick-sweep detection feeding the scoring bonus only — never a hard filter, off by default per Stage 1. |
| `CheckSpread` / `CheckSession` / news blackout | 7 | Optional spread ceiling, London/New York/Asian session windows, manual news blackout window. |
| `DrawSetup` / `DrawDashboard` | 2 (base) / 7 (scoring/risk fields) | Chart objects for BOS/RETEST/ENGULFING/BUY/SELL/SL/TP plus a live per-timeframe dashboard panel. |
| `SendAlert` | 2 | Routes to `Print`, `Alert`, and/or `SendNotification` depending on `Debug_Mode`/`Enable_Alerts`/`Enable_Push_Notifications`. |
| `ResetSetup` / `InvalidateSetup` | 2 | Returns a timeframe's state machine to `IDLE` after a trade or a cancellation, optionally clearing invalidated-setup drawings. |
